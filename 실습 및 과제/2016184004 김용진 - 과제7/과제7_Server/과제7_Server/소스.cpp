...
void worker_thread()
{
	...
	while (true) {
		...
		case OP_PLAYER_MOVE_NOTIFY:
			int player_id = over_ex->object_id;
			lua_State* L = g_clients[key].L;
			lua_getglobal(L, "event_player_move_notify");
			lua_pushnumber(L, player_id);
			lua_pushnumber(L, g_clients[player_id].x);
			lua_pushnumber(L, g_clients[player_id].y);
			lua_pcall(L, 3, 0, 0);
			delete over_ex; 		
			break;
...
int lua_send_chat_packet(lua_State* L)
{
	...
	int chatter_id = (int)lua_tointeger(L, -2);
...
int lua_add_npc_move_event(lua_State* L)
{
	...
	g_clients[npc_id].is_active = true;
	add_timer(npc_id, OP_RANDOM_MOVE, system_clock::now() + 1s);
...
void random_move_npc(int id)
{
	...
	// Error - vector iterators in range are from different containers
	...
	lua_getglobal(g_clients[id].L, "check_move_count");
	lua_pcall(L, 0, 0, 0);
